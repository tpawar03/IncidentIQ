...
- [x] **Pydantic contract layer**: Implement all models in `CONTRACTS.md §2`, with the `Citation.chunk_id` validator and the `CommandIntent` catalog validator. Unit tests for each validator (valid + invalid). _Done when: every state object round-trips and rejects bad input._ _New._
- [x] **Safe command catalog + deterministic renderer**: `catalog/commands.yml` + renderer that validates `command_id`/args/namespaces and fills templates by safe substitution. _Done when: a non-catalog `command_id` is impossible to execute; unsafe-action test = 0%._ _Risk-first: this is the safety backstop (FR-12/36, decision #11)._ _New._
- [x] **Corpus init + pgvector**: `init_corpus.py` seeds icco/postmortems + Scoutflo runbooks into pgvector via local `bge-base-en-v1.5`, with 500-token chunk cap and parent-child runbook chunking. Docker `depends_on` gate (FR-23). _Done when: corpus init completes <2 min; empty corpus never reachable._ _New._

## Core pipeline (single happy path, end to end)

- [x] **Ingestion slice**: `POST /api/v1/incidents` with the AlertManager v4 / PagerDuty Pydantic union, 202-in-<200ms + background task, raw-payload persistence (FR-01/02/04), and fingerprint dedup (FR-24). _Done when: paste a payload → record in Postgres <1s, 202 before any LLM call._ _Depends on: contract layer._ _New._
- [x] **Alert enricher**: attach owner/repo/deploys/`deploy_gap_minutes` + `traceback` from `public_annotations` (FR-03/20). Graceful nulls. _Done when: enriched `IncidentContext` produced; missing fields never block._ _New._
- [x] **Hybrid retriever**: concurrent BM25 + semantic (`asyncio.gather`), **RRF fusion + local cross-encoder rerank** to top-5, min-score threshold + weak-retrieval signal (FR-05/06/07, decision #5). _Done when: context recall >0.80 on 8 OTel golden scenarios._ _Depends on: corpus init._ _Risk: drives Layer B targets._ _New._
- [x] **RCA synthesizer + composite confidence**: grammar-constrained `RCAReport` with citations; **N=3 self-consistency** vote behind the semaphore; composite confidence formula + penalties (decisions #1/#7). Token budget manager (tiktoken, FR-25). _Done when: citations resolve to real `chunk_id`s; confidence is computed not emitted; no context overflow._ _Depends on: retriever, Ollama harness._ _New._

## Orchestration & routing (the graph)

- [x] **Flat LangGraph graph + Postgres checkpointer**: assemble nodes/edges per `AGENT_ORCHESTRATION.md §1`, with additive reducers for `errors`/`trace`, and Postgres checkpointing. _Done when: process killed mid-incident resumes from last node <10s (FR-33)._ _Risk-first: durability is hard to retrofit._ _Depends on: contract layer._ _New._ _(Walking-skeleton scope: retriever→rca_synthesizer wired + FR-33 durability proven; downstream nodes/gates in Task 10/11.)_
- [x] **Confidence gates + triage router**: deterministic routing functions; hybrid triage (rule prior + LLM confirm, disagreement→lower confidence); the two gates (`<0.65→escalate`, `<0.70→unknown`) (FR-08/10/11, decision #4). **(MF-1)** The 0.65/0.70 thresholds and the composite weights `w_self`/`w_ret` are **placeholders until the calibration task sets them** — do not hardcode final values here. _Done when: unit tests cover all branches incl. unknown default; triage accuracy >85% on labeled set; thresholds wired to config, not literals._ _Depends on: graph, RCA, calibration task._ _New._ _(Build + branch-coverage DONE 2026-06-29, 107 tests; structural disagreement→unknown; downstream targets are END placeholders → Tasks 11/12; triage-accuracy gate deferred to Layer A eval; threshold/weight VALUES pending calibration.)_
...
- [ ] **Catalog-authoring lint (closes F-21 gap)**: a startup/CI check over `catalog/commands.yml` that asserts the catalog's own safety invariants — every command declaring `allowed_namespaces` must also declare a `namespace` arg (else the renderer's namespace check silently no-ops, F-21), every template placeholder resolves to a declared arg, and every string arg reaching a template carries an `enum`/`pattern` (no bare `type: string`, F-18). _Done when: a catalog with a namespaced command missing its `namespace` arg fails the lint; a template referencing an undeclared placeholder fails; an unconstrained templated string arg fails._ _Depends on: catalog (Task 3)._ _Risk: the backstop currently trusts the catalog author for these; the lint makes that trust enforced._ _New (from Task 3 audit)._
